...
static inline void write595(uint8_t segments) {
  ...
  // Wywołujemy applyBrightness tylko, gdy sprzęt jest już zainicjowany
  if (g_hardwareReady) {
    applyBrightness(g_brightness);  // Włącz z powrotem z właściwą jasnością
  }
}
...
void TimeTask(void *pv) {
  ...
  for (;;) {
    ...
    if (getLocalTime(&ti, 0)) {  // 0 oznacza: sprawdź co masz w pamięci, nie czekaj
...
void LogicTask(void *pv) {
  ...
  for (;;) {
    // BEZPIECZNIK: Jeśli mamy czas, a flaga ID nadal wisi - wymuś start zegara
    ...
    if (!g_timeValid) {
      ...
      if (showTempNow && g_tempValid) {
        ...
        localBuf[0] = (g_hour >= 10) ? segForDigit(g_hour / 10) : FONT_BLANK;
        localBuf[1] = segForDigit(g_hour % 10);
        if (colon) localBuf[1] |= SEG_DP;
        localBuf[2] = segForDigit(g_minute / 10);
        localBuf[3] = segForDigit(g_minute % 10);
      }
...
void setup() {
  ...
  // sntp_set_sync_interval(60000); // 1 minuta
